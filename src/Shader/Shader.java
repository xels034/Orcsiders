...
public class Shader {
  ...
  private Map<String, ShaderVariable> vars = new HashMap<>();
   ...
   * TODO should this be called from <tt>finalize()</tt>, or is
...
//    return GL20.glGetShader(programID, GL20.GL_LINK_STATUS)==GL11.GL_TRUE;
  ...
  private void scrapeVariables(String varLine){
    ...
    String str;
    try (Scanner scanner = new Scanner(varLine);){
  ...
  private void scanSource(String filename){
    try(Scanner scanner = new Scanner(ResourceLoader.getResourceAsStream(filename));){
